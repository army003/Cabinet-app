...
import BiArrowRightShort from '@/assets/icons/BiArrowRightShort';
...
const buttonVariants = {
  ...
  secondary: tw`bg-button text-white rounded-s20 py-4 shadow-button hover:bg-button-hover transition-colors disabled:bg-button h-[50px]`,
...
const styles = {
  button: ({ variant = 'text', twStyle, loading }) => [
    ...
    twStyle && twStyle,
    variant === 'ghost' && loading && tw`h-[50px] py-0`,
    variant === 'secondary' && tw`py-0`
  ]
};
  ...
  return (
    <div tw='flex justify-between items-center space-x-2 w-full'>
      ...
      <div tw='flex flex-shrink-0 justify-center items-center w-1/5 max-w-[40px] h-[40px] bg-white text-black rounded-s14'>
        {loading ? <Loader variant='small' /> : icon}
      </div>
    </div>
  );
};
...
const SecondaryButton = ({ icon, text, variant = '', loading }) => {
  return (
    <div
      css={[tw`flex justify-center items-center space-x-2 flex-none relative`, variant === 'link' && tw`justify-start`]}
    >
      <BodyText text={text} variant='bold' twStyle={tw`line-clamp-1 text-s14`} />
      {loading ? <Loader variant='small' /> : icon}
    </div>
  );
};
